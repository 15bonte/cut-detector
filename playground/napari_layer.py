...
def main(
    ...
    # Colors list
    colors = np.array([[np.random.randint(0,255), np.random.randint(0,255), np.random.randint(0,255), 0.3] for i in range(100)])
    ...
    for mitosis_track in mitosis_tracks:
        ...
        mask_movie = np.stack([mask_movie,mask_movie,mask_movie,mask_movie],axis=-1)
